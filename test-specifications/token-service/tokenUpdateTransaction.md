...
| 1       | Updates a mutable token with an auto renew account                    | tokenId=<CREATED_MUTABLE_TOKEN_ID>, autoRenewAccount=<VALID_ACCOUNT_ID>      | The token update succeeds and the token has a valid auto-renew account.                  | N                 |
| 2       | Updates a mutable token with an auto renew account that doesn't exist | tokenId=<CREATED_MUTABLE_TOKEN_ID>, autoRenewAccount="123.456.789"           | The token update fails with an INVALID_AUTORENEW_ACCOUNT response code from the network. | N                 |
| 3       | Updates a mutable token with an empty auto renew account              | tokenId=<CREATED_MUTABLE_TOKEN_ID>, autoRenewAccount=""                      | The token update fails with an SDK internal error.                                       | N                 |
| 4       | Updates a mutable token with an auto renew account that is deleted    | tokenId=<CREATED_MUTABLE_TOKEN_ID>, autoRenewAccount=<DELETED_ACCOUNT_ID>    | The token update fails with an INVALID_AUTORENEW_ACCOUNT response code from the network. | N                 |
...
| 3       | Updates a mutable token with an expiration time of one less than the current time       | tokenId=<CREATED_MUTABLE_TOKEN_ID>, expirationTime=<CURRENT_TIME> - 1, commonTransactionParams.signers=[<CREATED_MUTABLE_TOKEN_ADMIN_KEY>]       | The token update fails with an INVALID_EXPIRATION_TIME response code from the network.                                  | N                 |
| 4       | Updates a mutable token with an expiration time 8,000,002 seconds from the current time | tokenId=<CREATED_MUTABLE_TOKEN_ID>, expirationTime=-8000002, commonTransactionParams.signers=[<CREATED_MUTABLE_TOKEN_ADMIN_KEY>]                 | The token update fails with an INVALID_EXPIRATION_TIME response code from the network.                                  | N                 |
